...
public class The3DClockView extends View {
    ...
    public The3DClockView(Context context) {
        this(context, null);
    }

    public The3DClockView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public The3DClockView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);


        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.The3DClockView, defStyleAttr, 0);
        mBackgroundColor = ta.getColor(R.styleable.The3DClockView_backgroundColor, Color.parseColor("#237EAD"));
        setBackgroundColor(mBackgroundColor);
        mLightColor = ta.getColor(R.styleable.The3DClockView_lightColor, Color.parseColor("#ffffff"));
        mDarkColor = ta.getColor(R.styleable.The3DClockView_darkColor, Color.parseColor("#80ffffff"));
        mTextSize = ta.getDimension(R.styleable.The3DClockView_textSize, sp2px(context, 14));
        ta.recycle();
        initPaintAndPath();
    ...
    private void initPaintAndPath() {
        ...
        //计算3D变换，并且生成一个Matrix，可以应用到Canvas上，这句话其实就是实现3D效果的核心原理。
    ...
    //重写ondraw，大部分比较难处理的自定义View的难点几乎在于此，要一点点的将东西画出来
    @Override
    ...
    private void drawTimeText() {
        ...
        * */
        ...
        for (int i = 0; i < 12; i++) {
            if (i == 0) {
                drawNum(mCanvas, i * 30, 12 + "", mTextPaint);
            } else {
                drawNum(mCanvas, i * 30, i + "", mTextPaint);
            }
        }
        mCanvas.restore();
        ...
        for (int i = 0; i < 12; i++) {
            mCanvas.drawArc(mCircleRectF, 5 + 30 * i, 20, false, mCirclePaint);
        }
    }
